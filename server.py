...
def disconnectClient(message, client, client_name):
    ...
    if(enter_client_name in clients):
        if( not clients[client_name]["connected_with"]):
            ...
            greet_message=f"Hello, {enter_client_name} you are disconnected with {client_name}."
            other_client_socket.send(greet_message.encode())

            message = f"You are disconnected with {enter_client_name}"

            client.send(message.encode())

        else:
            other_client_name = clients[client_name]["connected_with"]
            message=f"You are already connected with {other_client_name}"
...
def connectClient(message, client, client_name):
    ...
    if(enter_client_name in clients):
        if( not clients[client_name]["connected_with"]):
            ...
            message = f"You are succesfully connected with {enter_client_name}"
...
def handleClient(client, client_name):
    ...
    while True:
        try:
            ...
            chunks = client.recv(BUFFER_SIZE)
            message = chunks.decode().strip().lower()
            if(message):
